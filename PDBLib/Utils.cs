...
namespace PDBLib
{
    public static class Utils
    {
        ...
        public static PDBType
            StringizeTypeFast(uint type, Dictionary<uint, TypeInfo> tm, uint flags)
        {
            if (tm.TryGetValue(type, out TypeInfo ti))
            {
                var te = (TYPE_ENUM)(type & 0xff);
                var se = $"{te}";
                if ((type & (0x0600 | 0x0400)) != 0)
                    se += "*";
                var ie = ti.Type.ToString();
                var ve = ti.Data!=null ? string.Join(',', ToHex(ti.Data)):"";
                return new PDBType {  TypeName = se, TypeVal = ie, Value = ve };
            }
            return new PDBType();
        }
